import { Pressable, StyleSheet, TextInput, TouchableOpacity, KeyboardAvoidingView, Keyboard, ScrollView, Linking, Button} from 'react-native';
...
export default function Music({ route, navigation }: RootStackScreenProps<'Music'>) {
  ...
  {id: "3", name: "So Hot You're Hurting My Feelings", artist: "Caroline Polachek", added:"Marco", link:"https://open.spotify.com/track/5B6Kjha6RRIMWGN7zGsAaT?autoplay=true", image:"https://i.scdn.co/image/ab67616d0000b2737d983e7bf67c2806218c2759"},
  {id: "9", name: "Kilby Girl", artist: "The Backseat Lovers", added:"Asa", link:"https://open.spotify.com/track/1170VohRSx6GwE6QDCHPPH?autoplay=true", image:"https://f4.bcbits.com/img/a4171388314_5.jpg"},
  {id: "4", name: "All Too Well", artist: "Taylor Swift", added:"Max", link:"https://open.spotify.com/track/1q3RiD1tIWUpGsNFADMlvl?autoplay=true", image:"https://upload.wikimedia.org/wikipedia/en/4/47/Taylor_Swift_-_Red_%28Taylor%27s_Version%29.png"},
...
const toggleModal = () => {
  ...
  const addSong = (num:string) =>{
    var result = songs.map(el => el.id == num ? {...el, added: name} : el);
    changeSongs(result);
    console.log(result)
  };
  ...
  return (
    <View style={styles.container}>
        ...
        {songs.map((song: { id: string; name: string; artist: string; added:string; link:string; image:string}) => {
          if(song.added == ""){
          return(
            <Pressable key ={song.id} onPress={() => {addSong(song.id)}}>
          <View  style = {{height:70, flexDirection:"row", alignItems:"center", justifyContent:"space-between", width:"100%", marginVertical:5,}}>
            ...
            <View style = {{flexDirection:'column', justifyContent:"space-around", flex:2, width:"100%"}}>
                <Text numberOfLines={1} style ={[styles.text, {fontWeight:"900"}]}>{song.name}</Text>
                <Text> </Text>
                <Text numberOfLines={1}  style ={[styles.text, {fontWeight:"100", fontSize:14}]}>{song.artist}</Text>
            </View>
            <FontAwesome
                name="plus"
                size={30}
                color="black"
                style={{ marginRight: 15, alignSelf:"center",flex:0.3}}
              />
          </View>
          </Pressable>
        );
        ...
        <Pressable style={{borderRadius:10, backgroundColor:"#0F75B3", height:35, alignSelf:"center", marginEnd:10, flex:0, flexDirection:"column", alignItems:"center", margin:10}} onPress={toggleModal}>
            <Text style={{ margin: "auto", padding:10, alignSelf:"center", color:"white"}}>Done</Text>
          </Pressable>
        </View>
      </Modal>
      ...
      <View style = {{display:"flex", flexDirection:"row", alignItems: "center", width:"100%", justifyContent:"center"}}>
        ...
        <Pressable onPress={toggleModal} style={{right:0, position:"absolute"}}>
            <FontAwesome
                      name="plus"
                      size={35}
                      color="black"
      ...
      <View style={{position:"absolute", bottom:0, marginBottom:40, height:100, width:"100%", flexDirection:"row", justifyContent:"space-between"}}>
      <Pressable  style={{}}>
            <FontAwesome
                      name="backward"
                      size={45}
